...
AMI_ROOT=F:/data/DIHARD3/preprocess/ # root to AMI
FALIGN_ROOT=F:/data/DIHARD3/preprocess/ # root to forced alignment data, downloaded or custom
...
LABELS_DIR=F:/data/DIHARD3/labels # output dir for prepared label files
...
if [[ $stage -le 0 ]]; then
  ...
  python local/get_diarization_from_rttm.py --in_file $FALIGN_ROOT/$split/${split}.rttm --out_file data/diarization_${split}/${split}.json
