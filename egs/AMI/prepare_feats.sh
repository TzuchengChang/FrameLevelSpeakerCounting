...
AMI_ROOT=F:/data/ami/amicorpus # root to AMI
FALIGN_ROOT=F:/data/ami/falign # root to forced alignment data, downloaded or custom
...
LABELS_DIR=F:/data/ami/fa_labels # output dir for prepared label files
...
if [[ $stage -le 0 ]]; then
  ...
  python local/get_diarization_from_alignments.py --in_file $FALIGN_ROOT/$split/${FALIGN_TYPE}a_${split}_ali --out_file data/diarization_${split}/${split}.json
  ...
  python local/get_stats_from_diarization.py --diar_file data/diarization_${split}/${split}.json >> data/diarization_${split}/stats.txt
  done
fi
